...
- (void)sendPingWithData:(nullable NSData *)data withTTL:(int32_t)ttl_count;

/*! Sends a ping packet with a maximum number of hops (TTL).
 *  \details Sends a ping with a maximum TTL.
 *
 *  \param data Some data to include in the ping packet, after the ICMP header, or nil if you
 *      want the packet to include a standard 56 byte payload (resulting in a standard 64 byte
 *      ping).
...
- (void)simplePing:(SimplePing *)pinger didReceiveUnexpectedPacket:(NSData *)packet fromAddress:(NSData *)address;
